...
# 导出列（不含付款截图、合同）
XL_COLUMNS = ["序号", "发票PDF", "发票类型", "购买方名称", "纳税人识别号",
...
class ExportService:
    ...
    def _write_summary(self, ws, invoices: list[Invoice]):
        ...
        total_amt = sum(self._safe_float(inv.amount) for inv in invoices)
        total_tax = sum(self._safe_float(inv.tax_amount) for inv in invoices)
        total_all = sum(self._safe_float(inv.total) for inv in invoices)
    ...
    def _set_column_widths(self, ws):
        widths = [6, 26, 16, 20, 22, 20, 12, 8, 12, 14, 20, 14, 15, 14]
